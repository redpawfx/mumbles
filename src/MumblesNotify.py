...
## modifiied  3/30/09  to  change size of notifications  to fit  text in message  -johnc
#
#------------------------------------------------------------------------
...
class MumblesNotify(object):
	...
	def alert(self, plugin_name, title, message, image=None):
		...
		titleHeight  =  self.options.get_option(CONFIG_MT, 'text_title_height')
		notify_height = height + titleHeight
		...
		if cur_direction == CONFIG_NOTIFY_DIRECTION_DOWN:
			...
			if self.__n_index == 0:
				...
				new_y = self.__current_yBottom + spacing
		...
		self.__current_yBottom = new_y + notify_height
		...
		# show window for a defined about of time for certain windows  GROWLS and default mumbles notifications should  stay up until clicked
		if  (plugin_name != "GrowlMumbler" and plugin_name != "GenericMumbler"):
			try:
